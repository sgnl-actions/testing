...
import { buildCryptoMocks } from '../index.mjs';
...
async function main() {
  ...
  for (const idx of recordableIndices) {
    ...
    // Merge crypto declarations (scenario-level overrides action-level, per-function)
    const mergedCryptoDef = {
      ...(actionContext.crypto ?? {}),
      ...(scenario.context?.crypto ?? {})
    };
    ...
    // Replace raw crypto declarations with real async mock functions
